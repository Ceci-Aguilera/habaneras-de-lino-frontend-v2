...
const CategoryGrid = ({ id }) => {
    ...
    useEffect(() => {
        async function awaitCategory() {
            await CategoryF.fetchCategory(setCategory, id);
        }
        return () => {
            awaitCategory()
        }
    ...
    useEffect(() => {
        if (category !== null && category !== undefined && category.products.length > 0) {
            const new_products = []

            for (var i = 0; i < commonConstants.PAGINATION_SIZE; i++) {
                if (i < category.products.length) {
                    new_products.push(category.products[i]);
                }
                else {
                    break;
                }
    ...
    useEffect(() => {
        if (category !== null && category !== undefined) {
            ...
            if (currentFirstIndex < category.products.length) {
                for (var i = 0; i < commonConstants.PAGINATION_SIZE; i++) {
                    if (currentFirstIndex + i < category.products.length) {
                        new_products.push(category.products[currentFirstIndex + i]);
                    }
                    else {
                        break;
                    }
    ...
    const nextArrow = (e) => {
        e.preventDefault();
        if (currentFirstIndex + commonConstants.PAGINATION_SIZE < category.products.length) {
    ...
    return (loading) ? <div></div> : (
        <>
            ...
            <Banner image_source={category.image} fromPublic={false} tagLineBool={true}
                ...
                <div className={styles.collection_div}>
                    <div className={styles.title_div}>
                        ...
                        <h1 className={styles.title_h1_mobile}>{category.title}</h1>
                    </div>
                    ...
                    <div className={styles.grid_div}>
                        ...
                        <div className={styles.arrow_wrapper_div}>
                            ...
                            {(currentFirstIndex + commonConstants.PAGINATION_SIZE < category.products.length) ?
                                <div className={styles.arrow_div}>
